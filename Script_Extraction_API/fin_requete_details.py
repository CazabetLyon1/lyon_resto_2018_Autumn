...
def request_detail(nom_fichier_entree="a.json",nom_fichier_resultat="b.json",nom_fichier_erreur="c.json",API_KEY='AIzaSyD4vq_bMskdNlprQ3D1_6g32S6IYNV_iQM',num_sauv=0):
	
	lim=len(str(nom_fichier_entree))-4
	nom_fichier_a_continuer=nom_fichier_entree[0:lim]+"_"+str(float(num_sauv))+".txt"
	print(nom_fichier_a_continuer)
	#
	...
	with io.open(nom_fichier_a_continuer, 'r',encoding='utf8') as f:
		l=json.load(f)
	
	for resto in liste_resto:
		compteur+=1
		if compteur >= (num_sauv*50):
			...
			if dict_resultat["status"]!="OK" : 				#appel incorrect
				...
				if (dict_resultat.get("error_message",None) != None):
					message_erreur=dict_resultat["error_message"]
					print("Erreur : "+dict_resultat["error_message"] +"\n End")
				dict_point={}
...
request_detail(nom_fichier_entree="C:/Users/franc/restaurants.json",nom_fichier_resultat="restaurantsdetails.json",nom_fichier_erreur="erreurrestaurantdetails.json",API_KEY='AIzaSyA8cTQHnL8UOXwRHd1RN10tT2A33E8JxkA',num_sauv=50)
#api key rodi :AlzaSyA8cTQHnL8UOXwRHd1RN10tT2A33E8JxkA
#API_KEY moi 'AIzaSyD4vq_bMskdNlprQ3D1_6g32S6IYNV_iQM
