...
def decoupage(x,y,radius,radius_reel,liste_resto):
	...
	new_x=x+radius /2
	new_y=y+radius /(2)
	nb_res=nb_res_appel(new_x,new_y,radius_reel/(3/2),liste_resto)
	if nb_res > 50 :
		#print("appel rec en plus")
		decoupage(new_x,new_y,radius/(3/2),radius_reel/(3/2),liste_resto)
	else :
		if nb_res != 0:
			dict={}
			dict["lng"]=new_x
			dict["lat"]=new_y
			dict["rr"]=radius_reel/(3/2)
			dict["r"]=radius/(3/2)
			l.append(dict)
...
shrek(10,'liste_point_pour_restaurants_071218_final.txt',"C:/Users/franc/OneDrive/Documents/FAC/LIFPROJET/LIFPROJET/JSON/restaurants.json")
